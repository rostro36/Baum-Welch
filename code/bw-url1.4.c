		...
		for(int t = T-1; t > 0; t--){
			...
			for(int s = 0; s < hiddenStates ; s+=4){
				...
				for(int j = 0; j < hiddenStates; j+=4){
					...
					//s
					//j
					double temp00 = transitionMatrix[s*hiddenStates + j] * factor;
					a_new[s*hiddenStates+j] += alphat1Ns0 * temp00;
					beta_news0 += temp00;
					
					//j+1
					double temp01 = transitionMatrix[s*hiddenStates + j+1] * factor1;
					a_new[s*hiddenStates+j+1] += alphat1Ns0 * temp01;
					beta_news0 += temp01;
					
									
					//j+2
					double temp02 = transitionMatrix[s*hiddenStates + j+2] * factor2;
					a_new[s*hiddenStates+j+2] += alphat1Ns0 * temp02;
					beta_news0 += temp02;
					
					//j+3
					double temp03 = transitionMatrix[s*hiddenStates + j+3] * factor3;
					...
					//j
					double temp10 = transitionMatrix[(s+1)*hiddenStates + j] * factor;
					a_new[(s+1)*hiddenStates+j] += alphat1Ns1 * temp10;
					beta_news1 += temp10;
					
					//j+1
					double temp11 = transitionMatrix[(s+1)*hiddenStates + j+1] * factor1;
					a_new[(s+1)*hiddenStates+j+1] += alphat1Ns1 * temp11;
					beta_news1 += temp11;
					
									
					//j+2
					double temp12 = transitionMatrix[(s+1)*hiddenStates + j+2] * factor2;
					a_new[(s+1)*hiddenStates+j+2] += alphat1Ns1 * temp12;
					beta_news1 += temp12;
					
					//j+3
					double temp13 = transitionMatrix[(s+1)*hiddenStates + j+3] * factor3;
					...
					//j
					double temp20 = transitionMatrix[(s+2)*hiddenStates + j] * factor;
					a_new[(s+2)*hiddenStates+j] += alphat1Ns2 * temp20;
					beta_news2 += temp20;
					
					//j+1
					double temp21 = transitionMatrix[(s+2)*hiddenStates + j+1] * factor1;
					a_new[(s+2)*hiddenStates+j+1] += alphat1Ns2 * temp21;
					beta_news2 += temp21;
					
									
					//j+2
					double temp22 = transitionMatrix[(s+2)*hiddenStates + j+2] * factor2;
					a_new[(s+2)*hiddenStates+j+2] += alphat1Ns2 * temp22;
					beta_news2 += temp22;
					
					//j+3
					double temp23 = transitionMatrix[(s+2)*hiddenStates + j+3] * factor3;
					...
					//j
					double temp30 = transitionMatrix[(s+3)*hiddenStates + j] * factor;
					a_new[(s+3)*hiddenStates+j] += alphat1Ns3 * temp30;
					beta_news3 += temp30;
					
					//j+1
					double temp31 = transitionMatrix[(s+3)*hiddenStates + j+1] * factor1;
					a_new[(s+3)*hiddenStates+j+1] += alphat1Ns3 * temp31;
					beta_news3 += temp31;
					
									
					//j+2
					double temp32 = transitionMatrix[(s+3)*hiddenStates + j+2] * factor2;
					a_new[(s+3)*hiddenStates+j+2] += alphat1Ns3 * temp32;
					beta_news3 += temp32;
					
					//j+3
					double temp33 = transitionMatrix[(s+3)*hiddenStates + j+3] * factor3;
