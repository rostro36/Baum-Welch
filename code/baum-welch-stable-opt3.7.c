	...
	for(int t = 2; t < T-1; t++){
		...
		for(int i = 0; i<N; i+=block_size_y){
			for(int j = 0; j < N; j+= block_size_x){
				...
				for(int s = i; s < i + block_size_y; s++){
					double alphatNs = 0;
					for(int k = j; k < j + block_size_x; k++){
						//printf("hello %lf \n", alpha[(t-1)*N + k]);
						alphatNs += alpha[(t-1)*N + k] *a[s*N + k];
					}
					//alphatNs *= b[yt*N + s];
					//print_matrix(alpha,N,T);
					ctt += alphatNs;
					alpha[t*N + s] += alphatNs*b[yt*N + s];
				}
			}
		}
    ...
    */
	...
	for (int run=0; run<maxRuns; run++){
        ...
        */
		...
		final_scaling(transitionMatrix, emissionMatrix, stateProb, observations, gamma_sum, gamma_T,a_new,b_new,ct, indicator, hiddenStates, differentObservables, T);
