...
//Initial step
	...
	for(int t = T-1; t > 0; t--){
		...
		for(int s = 0; s < hiddenStates ; s+=4){
			...
			for(int j = 0; j < hiddenStates; j+=4){
				double factor = beta[j] * emissionMatrix[yt*hiddenStates + j];
				double factor1 = beta[j+1] * emissionMatrix[yt*hiddenStates + j+1];
				double factor2 = beta[j+2] * emissionMatrix[yt*hiddenStates + j+2];
				double factor3 = beta[j+3] * emissionMatrix[yt*hiddenStates + j+3];
				//s
				//j
				double temp = transitionMatrix[s*hiddenStates +j] * factor;
				double xi_sjt = alphat1Ns * temp;
				a_new[s*hiddenStates+j] +=xi_sjt;
				beta_news += temp;
				
				//j+1
				temp = transitionMatrix[s*hiddenStates + j+1] * factor1;
				xi_sjt = alphat1Ns * temp;
				a_new[s*hiddenStates + j+1] +=xi_sjt;
				beta_news += temp;
				
				//j+2
				temp = transitionMatrix[s*hiddenStates + j+2] * factor2;
				xi_sjt = alphat1Ns * temp;
				a_new[s*hiddenStates + j+2] +=xi_sjt;
				beta_news += temp;
				
				//j+3
				temp = transitionMatrix[s*hiddenStates + j+3] * factor3;
				xi_sjt = alphat1Ns * temp;
				...
				//j
				temp = transitionMatrix[(s+1)*hiddenStates +j] * factor;
				xi_sjt = alphat1Ns1 * temp;
				a_new[(s+1)*hiddenStates+j] +=xi_sjt;
				beta_news1 += temp;
				
				//j+1
				temp = transitionMatrix[(s+1)*hiddenStates + j+1] * factor1;
				xi_sjt = alphat1Ns1 * temp;
				a_new[(s+1)*hiddenStates + j+1] +=xi_sjt;
				beta_news1 += temp;
				
				//j+2
				temp = transitionMatrix[(s+1)*hiddenStates + j+2] * factor2;
				xi_sjt = alphat1Ns1 * temp;
				a_new[(s+1)*hiddenStates + j+2] +=xi_sjt;
				beta_news1 += temp;
				
				//j+3
				temp = transitionMatrix[(s+1)*hiddenStates + j+3] * factor3;
				xi_sjt = alphat1Ns1 * temp;
				...
				//j
				temp = transitionMatrix[(s+2)*hiddenStates +j] * factor;
				xi_sjt = alphat1Ns2 * temp;
				a_new[(s+2)*hiddenStates+j] +=xi_sjt;
				beta_news2 += temp;
				
				//j+1
				temp = transitionMatrix[(s+2)*hiddenStates + j+1] * factor1;
				xi_sjt = alphat1Ns2 * temp;
				a_new[(s+2)*hiddenStates + j+1] +=xi_sjt;
				beta_news2 += temp;
				
				//j+2
				temp = transitionMatrix[(s+2)*hiddenStates + j+2] * factor2;
				xi_sjt = alphat1Ns2 * temp;
				a_new[(s+2)*hiddenStates + j+2] +=xi_sjt;
				beta_news2 += temp;
				
				//j+3
				temp = transitionMatrix[(s+2)*hiddenStates + j+3] * factor3;
				xi_sjt = alphat1Ns2 * temp;
				...
				//j
				temp = transitionMatrix[(s+3)*hiddenStates +j] * factor;
				xi_sjt = alphat1Ns3 * temp;
				a_new[(s+3)*hiddenStates+j] +=xi_sjt;
				beta_news3 += temp;
				
				//j+1
				temp = transitionMatrix[(s+3)*hiddenStates + j+1] * factor1;
				xi_sjt = alphat1Ns3 * temp;
				a_new[(s+3)*hiddenStates + j+1] +=xi_sjt;
				beta_news3 += temp;
				
				//j+2
				temp = transitionMatrix[(s+3)*hiddenStates + j+2] * factor2;
				xi_sjt = alphat1Ns3 * temp;
				a_new[(s+3)*hiddenStates + j+2] +=xi_sjt;
				beta_news3 += temp;
				
				//j+3
				temp = transitionMatrix[(s+3)*hiddenStates + j+3] * factor3;
				xi_sjt = alphat1Ns3 * temp;
...
//BAUM-WELCH
	        ...
	        for(int v = 0; v < differentObservables; v+=4){
		        for(int s = 0; s < hiddenStates; s+=4){
					double gamma_sums_inv=1.0/gamma_T[s];
					double gamma_sums_inv1=1.0/gamma_T[s+1];
					double gamma_sums_inv2=1.0/gamma_T[s+2];
					double gamma_sums_inv3=1.0/gamma_T[s+3];
					//v,s
