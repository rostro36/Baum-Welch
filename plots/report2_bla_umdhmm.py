...
file_name_bla = "*bla-time*"
full_name_bla =glob.glob(folder + file_name_bla)

file_name_umdhmm = "*umdhmm-time*"
full_name_umdhmm =glob.glob(folder + file_name_umdhmm)
...
for file in flags.keys():
    for flag in flags[file].keys():
        ...
        for n in flags[file][flag]:
            ...
            if(file == 'vec-op'):
                filename = 'umd'
        plt.plot(x,y, marker=markers[marker], color=comp_colors[color], linestyle=styles[style], label= filename+': '+ compiler+' ' +str(flag))
        color+=2
        marker+=2
        if(file=='bla'):
           marker+=2
